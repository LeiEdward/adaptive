...
include_once ($_SERVER['DOCUMENT_ROOT']."/htdocs/ntcu-nbnat-AIAL/include/config.php");
require_once ($_SERVER['DOCUMENT_ROOT']."/htdocs/ntcu-nbnat-AIAL/include/adp_core_function.php");
require_once ($_SERVER['DOCUMENT_ROOT'].'/htdocs/ntcu-nbnat-AIAL/include/ref_cityarea.php');

$sSemeYear = '1061';//getYearSeme();

echo 'path:'.$_SERVER['DOCUMENT_ROOT'].',sems:'.$sSemeYear;
...
function createReport_dailyusage($sSemeYear) {
  global $dbh, $ref_cityarea;
   ...
   if (0 < $iReprotCount) {
     file_put_contents($_SERVER['DOCUMENT_ROOT'].'/php_cli/rs_dailyusage.log', date('Y-m-d H:i:s').' : 今天已經執行過報表'.PHP_EOL, FILE_APPEND);
     return;
   }

file_put_contents($_SERVER['DOCUMENT_ROOT'].'/php_cli/rs_dailyusage.log', date('Y-m-d H:i:s').' : 正在查詢影片瀏覽人數...'.PHP_EOL, FILE_APPEND);
  // 影片瀏覽人數
...
file_put_contents($_SERVER['DOCUMENT_ROOT'].'/php_cli/rs_dailyusage.log', date('Y-m-d H:i:s').' : 正在查詢影片瀏覽時間...'.PHP_EOL, FILE_APPEND);
  // 影片瀏覽時間
...
file_put_contents($_SERVER['DOCUMENT_ROOT'].'/php_cli/rs_dailyusage.log', date('Y-m-d H:i:s').' : 正在查詢練習題人數...'.PHP_EOL, FILE_APPEND);
  //練習題人數 5
...
file_put_contents($_SERVER['DOCUMENT_ROOT'].'/php_cli/rs_dailyusage.log', date('Y-m-d H:i:s').' : Create View Table...'.PHP_EOL, FILE_APPEND);
  // Create View
...
file_put_contents($_SERVER['DOCUMENT_ROOT'].'/php_cli/rs_dailyusage.log', date('Y-m-d H:i:s').' : Query View Table...'.PHP_EOL, FILE_APPEND);
...
file_put_contents($_SERVER['DOCUMENT_ROOT'].'/php_cli/rs_dailyusage.log', date('Y-m-d H:i:s').' : Data Handling...'.PHP_EOL, FILE_APPEND);
  $vReportData = array();
  // 影片瀏覽人數
...
file_put_contents($_SERVER['DOCUMENT_ROOT'].'/php_cli/rs_dailyusage.log', date('Y-m-d H:i:s').' : Query LearningStat...'.PHP_EOL, FILE_APPEND);
  $vReportData = getLearningStat($vReportData, $sSemeYear);

file_put_contents($_SERVER['DOCUMENT_ROOT'].'/php_cli/rs_dailyusage.log', date('Y-m-d H:i:s').' : Insert report_dailyusage...'.PHP_EOL, FILE_APPEND);
...
file_put_contents($_SERVER['DOCUMENT_ROOT'].'/php_cli/rs_dailyusage.log', date('Y-m-d H:i:s').' : Finish report_dailyusage'.PHP_EOL, FILE_APPEND);
